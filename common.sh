...
function settings_variable() {
  ...
  bash <(curl -fsSL https://raw.githubusercontent.com/hunya2017/MYSSOpenWrt/refs/heads/main/common.sh)
}
...
function Diy_update() {
  bash <(curl -fsSL https://raw.githubusercontent.com/hunya2017/MYSSOpenWrt/refs/heads/main/custom/ubuntu.sh)
  if [[ $? -ne 0 ]]; then
...
function Diy_checkout() {
  ...
  gitsvn https://github.com/hunya2017/MYSSOpenWrt/tree/main/auto-scripts ${HOME_PATH}/package/auto-scripts
  ...
  if [[ -z "$(find "$A_PATH" -type d -name "default-settings" -print)" ]] && [[ -d "$C_PATH" ]]; then
    gitsvn https://github.com/hunya2017/MYSSOpenWrt/tree/main/Share/default-settings "${HOME_PATH}"/package/default-settings
    ...
    gitsvn https://github.com/hunya2017/MYSSOpenWrt/tree/main/Share/default-setting "${HOME_PATH}"/package/default-settings
    if grep -q "libustream-wolfssl" "${HOME_PATH}/include/target.mk"; then
      sed -i 's?libustream-wolfssl?libustream-openssl?g' "${HOME_PATH}/include/target.mk"
    fi
    if ! grep -q "dnsmasq-full" "${HOME_PATH}/include/target.mk"; then
...
luci-app-wechatpush,v2ray-core,v2ray-plugin,v2raya,xray-core,xray-plugin,luci-app-alist,alist"
  ...
  if [[ "${REPO_BRANCH}" == *"18.06"* ]] || [[ "${REPO_BRANCH}" == *"19.07"* ]] || [[ "${REPO_BRANCH}" == *"21.02"* ]] || [[ "${REPO_BRANCH}" == *"22.03"* ]]; then
    gitsvn https://github.com/hunya2017/MYSSOpenWrt/blob/main/Share/shadowsocks-rust/Makefile ${HOME_PATH}/feeds/danshui/luci-app-ssr-plus/shadowsocks-rust/Makefile
  fi
  ...
  if [[ ! -d "${HOME_PATH}/feeds/packages/devel/packr" ]]; then
    gitsvn https://github.com/hunya2017/MYSSOpenWrt/tree/main/Share/packr ${HOME_PATH}/feeds/packages/devel/packr
  fi
...
function Diy_OFFICIAL() {
  ...
  if [[ "${REPO_BRANCH}" =~ (main|master|openwrt-24.10) ]]; then
    gitsvn https://github.com/hunya2017/MYSSOpenWrt/blob/main/Share/luci-app-nginx-pingos/Makefile ${HOME_PATH}/feeds/danshui/luci-app-nginx-pingos/Makefile
  fi
}
...
' >>${HOME_PATH}/.config
    gitsvn https://github.com/hunya2017/MYSSOpenWrt/blob/main/Share/block/10-mount ${HOME_PATH}/files/etc/hotplug.d/block/10-mount
  fi
...
function Diy_prevent() {
  ...
  if [[ $(grep -c "CONFIG_PACKAGE_luci-theme-argon=y" ${HOME_PATH}/.config) -eq '1' ]]; then
    ...
    curl -fsSL https://raw.githubusercontent.com/hunya2017/MYSSOpenWrt/refs/heads/main/Share/argon/jpg/${pmg}.jpg -o ${HOME_PATH}/files/www/luci-static/argon/background/argon.jpg
    if [[ $? -ne 0 ]]; then
      echo "拉取文件错误,请检测网络"
      exit 1
    fi
...
function Diy_adguardhome() {
  ...
  if [[ ! "${weizhicpu}" == "1" ]] && [[ "${AdGuardHome_Core}" == "1" ]]; then
    ...
    wget -q https://github.com/hunya2017/MYSSOpenWrt/releases/download/API/AdGuardHome.api -O AdGuardHome.api
    if [[ $? -ne 0 ]]; then
      curl -fsSL https://github.com/hunya2017/MYSSOpenWrt/releases/download/API/AdGuardHome.api -o AdGuardHome.api
    fi
...
function Package_amlogic() {
  ...
  if [[ -z "${amlogic_kernel}" ]]; then
    curl -fsSL https://github.com/hunya2017/MYSSOpenWrt/releases/download/API/${kernel_usage}.api -o ${HOME_PATH}/${kernel_usage}.api
